            ...
            {props.galleries.map((item, index) => (
                 <div key={item.itemId} className='nft-list__item w-full h-auto grid grid-cols-1 bg-[#12101c] border-2 border-[#161A42] dark:bg-white rounded-2xl text-white dark:text-gray-800 dark:border-2 dark:border-gray-200'>
                    <div className='nft-list__item-wrap grid grid-cols-1 p-4'>
                        ...
                        <p className="nft-item__creator-title text-sm truncate ... ">{item.creator} </p>
                        <p className="nft-item__creator-address text-[#A2A6D0] text-xs">Creator</p>
                    </div>                            
                    
                    <div className="nft-item__buy-wrap">
                        <button className='my-button buy-button rounded-full bg-gradient-to-b from-[#3461FF] to-[#8454EB] text-white text-base w-full px-4 py-2 shadow-md' onClick={()=>props.children.cancelSellFucnction(item)}>{item.buttonTitle}</button>
                    </div>
