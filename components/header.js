...
useEffect(()=>{
    ...
    return (
        ...
        <Disclosure as="nav" className="header z-50 fixed w-full h-[70px] transition-all bg-[#12121E] dark:bg-white dark:border-b dark:border-gray-200 border-[#353437] border-b-2">
        {({ open }) => (
            <div className='w-full 2xl:max-w-screen-2xl h-full mx-auto'>
                <div className="mx-auto px-4 sm:px-8 lg:px-[6vw] h-full">
                    <div className="relative flex items-center justify-between h-full">
                        <div className="hamburger-menu absolute inset-y-0 left-0 flex items-center lg:hidden">
                            {/* Mobile menu button*/}
                        ...
                        <div className="flex-1 flex items-center justify-center lg:items-stretch lg:justify-start h-full">
                            <div className="header__left-side flex-none self-center">
                                <Link href="/">
                                    <a>
                                        <img src={"/assets/svg/logo-" + (themeMode ? "dark" : "light") + ".svg"} alt="Logo" className='header__logo h-35px w-auto'></img>
                                    </a>
                                </Link>
                            </div>
                            <div className="header__right-side lg:block lg:ml-70px flex-1 h-full">
                                <div className="items-center flex xl:space-x-4 h-full">
                                    {headers.map((item, index) => (
                                    <div key={item.name} className={classNames('flex items-center ', index === currentPage ? 'text-white dark:text-gray-800 nav__link_active nav__link' : 'nav__link text-gray-200 dark:text-gray-600', 'hover:text-gray-400 dark:hover:text-gray-400 font-bold')}>
                                        <Link href={item.href}>
                                            <a
                                    ...
                                    {/* dark or light mode */}
                                    <div className='color-mode-button relative grid place-items-center'>
                                        <Switch checked={themeMode} onChange={toggleThemeMode} className={classNames(themeMode ? 'bg-gray-800' : 'bg-[#8B8DA1]', 'transition duration-150 ease-out relative inline-flex items-center h-6 rounded-full w-12')}>
                                   ...
                                   {account? <Menu as="div" className="relative inline-block text-left">
                                        <div className='h-full grid place-items-center'>
                                            <Menu.Button className="inline-flex justify-center w-full px-0 py-2 text-sm font-medium">
                                                <div className='header__account-wrap flex flex-row space-x-4'>
                                                    <div className='header__account-ava flex-none w-12 h-12 rounded-full bg-gradient-to-b from-[#FF2D92] to-[#FFA25F]'></div>

                                                    <div className='header__account-info flex flex-col '>
                                                        <p className='header__account-address flex-1 text-white dark:text-gray-800 text-sm font-bold cut-wallet'> {account}</p>
                                                        <p className='header__account-balance  text-[#7D82B2] dark:text-gray-800 text-xs text-left'>{accountBalance} Matic</p>
                                                    </div>
                                                </div>
                                            </Menu.Button>
                                        </div>
                                        ...
                                        >
                                        <Menu.Items className="user-dropdown absolute right-0 w-80 mt-4 p-8 origin-top-right shadow-lg">
                                                    ...
                                                    <Menu.Item>
                                                        <div className='user-dropdown__account flex flex-row space-x-3'>
                                                            ...
                                                            <div className='user-dropdown__account-balance'>
                                                                <p className='cut-wallet'>Matic Balance </p>
                            ...
                            <>
                                <Disclosure.Button className="hidden-nav__account-wrap flex justify-between w-full px-4 py-2 text-sm font-medium text-left focus:outline-none focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75">
                                    <div className='flex flex-row space-x-4'>
                                        <div className='hidden-nav__account-ava flex-none w-12 h-12 rounded-full bg-gradient-to-b from-[#FF2D92] to-[#FFA25F]'></div>
                                ...
                                <Disclosure.Panel className="pt-4 pb-2 text-sm text-gray-500">
                                    <div className='flex flex-col space-y-6'>
                                            ...
                                            <a>
                                                <div className='user-dropdown__account flex flex-row space-x-3'>
                                                    ...
                                                    <div className='user-dropdown__account-balance flex-1 flex flex-col '>
                                                        {/* <p className='flex-1 text-white dark:text-gray-800 text-sm font-bold cut-wallet'>Account Address</p>
                                        ...
                                        <Link href="/market/nft-resell">
                                            <a className="user-dropdown__link">
                                                <div className='flex flex-row items-center space-x-4 text-white dark:text-gray-800'>
                                                    <span className='flex-1 text-sm'>NFT Resell
                                                    </span>
                                                </div>
                                            </a>
                                        </Link>
