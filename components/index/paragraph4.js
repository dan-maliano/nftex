  ...
  if (category === "ALL") {
    ...
    return (
        <div className='w-full h-auto'>
            <div className='relative grid grid-cols-1 gap-8 m-4 sm:m-16 lg:m-[9vw]'>
                {/* categories */}
                <div className="flex flex-col space-y-12">
                    <h1 className="second-title">Explore all NFTs and Start to Collect</h1>
                    <div className='filters-wrap flex flex-col md:flex-row space-y-2 md:space-y-0'>
                        <div className='filters-items-wrap flex flex-col md:flex-row space-y-2 md:space-x-2 md:space-y-0'>
                            {btnCategories.map((item, index) => (
                                <button key={"btn-category" + index.toString()} className={classNames(index === current ? 'filter-button filter-button_active' : ' filter-button', '')} onClick={() => {setCurrent(index)
                                    setCategory(item)
                                }}>{item}</button>
                            ))}
                        </div>
